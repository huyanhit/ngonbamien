...
class StorageService extends Service{
    ...
    public function generateReport(){
        $allProducts   = ProductModel::join('product_option','product_option.product_id','=', 'products.id')->where('active', 1)->get();
        ...
        $productExportWeek = OrderModel::whereBetween('created_at', 
        ...
        $productExportMonth = OrderModel::whereBetween('created_at', 
            [Carbon::now()->startOfMonth(), Carbon::now()])->get()->sum(function($item){
            return $item->products->sum(function($item){
                return $item->pivot->quantity;
            });
        });
        $productExportYear  = OrderModel::whereBetween('created_at', [Carbon::now()->startOfYear(), Carbon::now()])->get()->sum(function($item){
            return $item->products->sum(function($item){
                return $item->pivot->quantity;
            });
        });
        ...
        return [
            ...
            'supplier_count'       => SupplierModel::where('active', 1)->count(),
        ];
    }
    ...
    public function storeQuery(){
        ...
        if($query){
            switch($query['storage_report']){
                ...
                case '2':
                    if($query['picker_from'] && $query['picker_to']){
                        $ids = OrderModel::whereBetween('created_at',
                ...
                break;
                case '3':
                    $this->model = $this->model->where('product_option.date_ex', '<', Carbon::now()->addDays($query['storage_number']?? 0));
                break;
                case '4':
                    $this->model = $this->model->where('product_option.stock','<', $query['storage_number']?? 0);
                break;
                case '5':
