    ...
    public function handle(
        ...
        return DB::transaction(function () {
            if ($this->messageService->updateMessage($this->messageId, $this->data)) {
                ...
                $message = $this->messageService->getChatRepository()->getObject('MESSAGE', $this->data['room_id'] . '_' . $this->messageId);
                ...
                if(isset($this->data['thread']) && ($this->data['thread'] != 0)){
                    $key = $this->data['thread'].'_'.$this->messageId;
                    $thread = $this->messageService->getChatRepository() ->getObject('THREAD', $key);
                    $result = array_merge($files, $thread);
                }else{
                    $result = array_merge($files, $message);
                }
