...
class ContactService extends ChatService
{
    ...
    public function addContact($data): array
    {
        return DB::transaction(function () use ($data){
            ...
            NotificationService::pushNotification('chat_add_contact', [
                'tos' => $data['ids'],
                'contact_id' => $authId
            ]);
    ...
    public function approve($data): array
    {
        $results = DB::transaction(function () use ($data) {
            ...
            // push notification
            NotificationService::pushNotification('chat_approve_contact', ['tos' => $data['ids']]);
        ...
        if($results){
            ...
            foreach ($results as $item){
                ...
                $this->dispatchSync((new SendMessage([
                    ...
                    'auth' => Auth::id(),
                    'content' => "[room-create][auth:" . Auth::id() . "][room:" . $item['room_id'] . "][room-create]".
                        "[room-add-member][auth:" . Auth::id() . "][members:".Auth::id()."-".$item['user_id']."][room-add-member]"
                ]))->onQueue('message'));
            }
