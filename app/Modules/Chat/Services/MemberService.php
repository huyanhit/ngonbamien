...
class MemberService extends ChatService
{
    ...
    public function getMembers($roomId): array
    {
        ...
        $users = $this->getChatRepository()->getObjectsByList('USER', $listMember['ROOM_MEMBER'][$roomId]);
        return array_merge($users, $listMember, $this->getChatRepository()->getObjectsByList('MEMBER', $listMember['ROOM_MEMBER'][$roomId], $roomId));
    }
    ...
    public function updateUnread($data, $authId){
        $mention     = 0;
        $key         = $data['room_id'].'_'.$authId;
        ...
        foreach ($messages['MESSAGE'] as $message) {
            $hasMention = $this->checkContentHasMention($data['room_id'], $authId, $message['content']);
