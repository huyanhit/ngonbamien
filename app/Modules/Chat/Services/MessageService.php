...
class MessageService extends ChatService
{
    ...
    public function getMessages($data){
        if(empty($data['type'])){
            ...
            }elseif($data['type'] == self::GET_MORE_MESSAGES_DOWN){
    ...
    public function addThread($data){
        if(isset($data['thread'])){
            $key = $data['room_id'].'_insert_thread';
            if(!Cache::has($key)){
                Cache::put($key, $this->getChatRepository()->getLenOfList('MESSAGE_THREAD', $data['thread']),60);
            }
    ...
    public function addMessage($data){
        ...
        if(!Cache::has($key)){
            Cache::put($key, $this->getChatRepository()->getLenOfList('ROOM_MESSAGE', $data['room_id']),60);
        }
        $data = array_merge($data, ['status' => 1, 'id' => Cache::increment($key)]);
    ...
    public function updateMessage($messageId, $data){
        if(isset($data['thread']) && ($data['thread'] != 0)){
            $key = $data['thread'].'_'.$messageId;
            return $this->getChatRepository()->updateObject('THREAD', $key, $data);
        }else{
    ...
    public function deleteMessage($messageId, $data){
        if(isset($data['thread']) && ($data['thread'] != 0)){
            $key = $data['thread'].'_'.$messageId;
            return $this->getChatRepository()->deleteObject('THREAD', $key);
        }else{
            $key = $data['room_id'].'_'.$messageId;
            return $this->getChatRepository()->deleteObject('MESSAGE', $key);
        }
    }
