...
function mySticky() {
    ...
    document.onscroll = function() {
        if (header !== null) {
            let sticky = header.offsetTop + 47;
...
function updateCartDom(){
    let html =
        ...
        '<th width="10%" class="text-center dis_none">Shop</th>' +
        '<th width="20%" class="">Tên sản phẩm</th>' +
        '<th width="10%" class="text-center dis_none">Phân loại</th>' +
        '<th width="15%" class="text-center dis_none">Giá</th>' +
        '<th width="10%" class="text-center">Số lượng</th>' +
        '<th width="15%" class="text-center">Tổng cộng</th>' +
        '<th width="5%" class="text-center">Xoá</th>' +
        '</tr>';

    let groups = Object.groupBy(Object.values(cartDom.items), (item) => {
    ...
    if(Object.keys(groups).length > 0){
        for (const index in groups) {
            let ship           = 15000;
            let sPriceQuantity = 0; 
            let freeShip  = [];
            let downPrice = [];
            let items  = groups[index];
            for (const index in items) {
                sPriceQuantity += items[index].price * items[index].quantity;
                let optionHtml = '';
                ...
                html +=
                    ...
                    '<td class="text-center align-middle dis_none">'+ items[index].extra_info.shop?.name +'</td>' +
            ...
            let supports = items[0].extra_info.shop.support;
            let ships    = items[0].extra_info.shop.ship;
            let spHtml   = '';
            ...
            if(supports.length){
                ...
                for (const key in supports) {
                    switch(supports[key].support_id){
                        case 1:
                            active = sPriceQuantity >= supports[key].value_1;
                            freeShip[supports[key].supplier_id] = active;
                            sShip     -= active? ship: 0;
                            acHtml     = active?'border-success text-success':'border-secondary text-muted';
                            spHtml    += '<span class="badge badge border '+ acHtml +' mx-1">'+
                                         '<i class="fa fa-check-square-o '+ acHtml +'"></i> Miễn phí ship đơn từ '+ 
                                         VND.format(supports[key].value_1) +'</span>';
                        break
                        case 2:
                            active = sPriceQuantity >= parseInt(supports[key].value_2);
                            downPrice[supports[key].supplier_id] = active? supports[key].value_1: 0; 
                            sdownPrice += active? parseInt(supports[key].value_1): 0;
                            acHtml      = active?'border-success text-success':'border-secondary text-muted';
                            spHtml     += '<span class="badge badge border '+ acHtml +' mx-1">'+
                                          '<i class="fa fa-times-circle-o '+ acHtml +'"></i> Giảm '+ 
                                          VND.format(supports[key].value_1) +' cho đơn từ '+ VND.format(supports[key].value_2) +'</span>';
                        break
                    }
                }
            }
            let fsHtml = freeShip[items[0].extra_info.supplier]?'<span class="badge badge border border-success text-success mx-1"> Miễn phí </span>':VND.format(ship);
            let dpHtml = downPrice[items[0].extra_info.supplier]? VND.format(downPrice[items[0].extra_info.supplier]):VND.format(0);
            html += 
                '<tr class="align-middle alert alert-secondary">'+
                    '<td class="text-center dis_none" style="vertical-align: middle;"><a href="shop/'+items[0].extra_info.shop.slug+'">' +
                        '<img onerror="this.src=\'/images/no-image.png\'" ' +
                        'src="/admin/get-image-thumbnail/'+items[0].extra_info.shop.image_id+'"/><a></td>' +
                    '<td class="text-center align-middle dis_none" colspan="5">'+
                        '<div class="row align-items-center">'+
                        '    <div class="col-sm-2 small"><div>Cửa hàng</div> '+ items[0].extra_info.shop.name+'</div>'+
                        '    <div class="col-sm-2 small"><div>Phí vận chuyển</div> '+ fsHtml +'</div>'+
                        '    <div class="col-sm-2 small"><div>Cửa hàng Giảm</div> '+ dpHtml +'</div>'+
                        '    <div class="col-sm-6 small">'+ spHtml +'</div>' +
                        '</div>' +
                    '</td>' +
                    '<td class="text-center align-middle dis_none">'+ VND.format(sPriceQuantity) +'</td>' +
                    '<td class="text-center align-middle dis_none"></td>' +
                '</tr>'
        }
    }else{
        html +=
    ...
    html +=
        '<tr class="font-bold background_pr text-white">'+
            '<td ><h5 class="text-uppercase text-center">Tổng</h5></td>' +
            '<td class="dis_none text-center" colspan="2">Ship: '+ VND.format(sShip) +'</td>' +
            '<td class="dis_none text-center" colspan="2">Giảm: '+ VND.format(sdownPrice) +'</td>' +
            '<td class="text-center"><h5 class="text-bold">SL: '+ cartDom.quantities_sum +'</h5></td>' +
            '<td class="text-center"><h5 class="text-bold">'+ VND.format(cartDom.subtotal) +'</h5></td>' +
    ...
    $('#total-ship').html('+ ' + VND.format(sShip));
    $('#total-down').html('- ' + VND.format(sdownPrice));
    $('#coupon-down').html('- ' + VND.format(coupon));
    $('#total-check').html(VND.format(cartDom.total + sShip - sdownPrice - (coupon ? parseInt(coupon) : 0)));
}
...
function chat(e, command, data) {
    let token = localStorage.getItem('cbox_token');
    if(token){
        $.ajax({
            type: 'POST',
            url: '/cbox/message-command',
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('cbox_token'),
        ...
        document.getElementById('cbox-btn').click();
    }
}
...
$(document).ready(function () {
    ...
    $('.add_cart').on('click', function(e) {
        ...
        let optionValue = $(this).attr('option-value');
        if(!optionValue){
            optionValue = $(this).parents('.featured__item').find('.product__item__price:not(.hide)').attr('data-value');
        }
        if (optionValue){
            flyToElement($(itemImg), $('.cart_anchor'));
            addCart(this, {id:$(this).attr('data-value'), option:optionValue}, $(this).attr('data-link')??'');
        }
    });
    ...
    $('.as_message').on('click', function(e) {
        e.preventDefault();
        chat(this, 'chat_product', {id:$(this).attr('data-value')});
    });
    $('.shop_chat').on('click', function(e) {
        e.preventDefault();
        chat(this, 'chat_shop', {id:$(this).attr('data-value')});
    });
    ...
    $("#sendSubscribe").on('submit', function (event) {
        const emailInput = document.getElementById('email_subscribe');
        const notice = document.getElementById('subscribe_notice');
        if (emailInput.checkValidity()) {
            ...
            $.ajax({
                ...
                notice.innerHTML  = "<span class='text-success'>Đăng ký thành công với Email: "+emailInput.value+" </span>"
                emailInput.value  = '';
            }).fail(function(){
                notice.innerHTML  = "<span class='text-danger'>Email đã được đăng ký rồi. Hãy nhập Email khác. </span>"
                emailInput.value  = '';
            })
        }
    })
